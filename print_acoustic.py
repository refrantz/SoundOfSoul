...
def train_and_assign_kmeans(points, num_clusters, centroids_file, indices_file):
    """
    ...
    # Write centroids to the text file
    with open(centroids_file, 'w') as f:
        for centroid in centroids:
            f.write(" ".join(map(str, centroid)) + "\n")
    
    # Write cluster indices to the text file
...
def process_song(metadata_path, stems_dir):
    ...
    for idx, (mixed_audio, input_audio) in enumerate(segments):
        #semantical_input = extract_layer_embeddings(input_audio)
        acoustical_output = encodec(input_audio, input_sample_hz=16000)[1]
        ...
        #all_output_size.append(semantical_input.shape[0])

        # Save to text files, one token per line, appending the segment index to filenames
        np.savetxt(f"acoustical_tokens/double_bass_input/{song_name}_segment_{idx}.txt", acoustical_output, fmt='%f')
        print(f"Processed and saved data for {song_name} segment {idx}")

def write_all(output_file):
    global all_output_size

    # Flatten and write all acoustical output to a single file
    with open(output_file, 'w') as f:
        for token in all_output_size:
            f.write(f"{token}\n")
